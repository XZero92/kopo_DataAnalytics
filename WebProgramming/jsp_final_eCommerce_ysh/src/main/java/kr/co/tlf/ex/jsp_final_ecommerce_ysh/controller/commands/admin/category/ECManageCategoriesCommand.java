...
public class ECManageCategoriesCommand implements ECCommand {
    @Override
    public void execute(HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            // 카테고리 관리 페이지로 포워딩
            RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/views/admin/manage_categories.jsp");
            dispatcher.forward(request, response);
        } catch (ServletException | IOException e) {
            System.err.println("카테고리 관리 페이지 처리 중 오류 발생: " + e.getMessage());
            e.printStackTrace();
        }
    }
}